...
const AllProperties = () => {
  ...
  const currentFilterValues = useMemo(() => {
    const logicalFilters = filters?.flatMap((item: any) => ("field" in item ? item : [])) || [];

    return {
      title: logicalFilters.find((item: any) => item.field === "title")?.value || "",
      propertyType: logicalFilters.find((item: any) => item.field === "propertyType")?.value || "",
    };
  }, [filters]);
      ...
      {allProperties.length > 0 && (
        <Box display="flex" gap={2} mt={3} flexWrap="wrap">
          ...
          <Box display={{ xs: 'none', sm: 'flex' }} alignItems="center" gap="5px">
            Page{' '}<strong>{currentPage} of {pageCount}</strong>
          </Box>
          <CustomButton
            title="Next"
            handleClick={() => setCurrentPage((prev: number) => prev + 1)}
